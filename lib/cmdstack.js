...
function parseRow(line, isLast) {
  ...
  const arrowHtml = isLast ? "" : '<span class="cmdstack-arrow" aria-hidden="true"></span>';
...
export function renderCmdstack(source) {
  ...
  const sectionHtml = sections
    .map((lines) => {
      const rows = lines.map((line, i) => parseRow(line, i === lines.length - 1)).filter(Boolean).join("");
